...
*/
  ...
  } else {  unset($_SESSION['marketname']);  unset($marketname); };
     ...
     <?php include("includes.php");  ?>
      <body class="hold-transition skin-blue sidebar-mini ownfont">
        ...
        <div class="content">
          ...
          <div class="col-md-6 panel-info" style="height:470px; background-color: yellow">
            left side panel
            <div class="panel-heading"><b>My List</b></div>
              <div class="panel-body">
                <div class="col-md-12" style="background-color: lightgreen">
                   my device create box
                ...
                <div id="switchlist" class="col-md-6" style="background-color: skyblue">    
                  My switch list panel              
                 <div style="border:1px solid darkgray;" > 
                    <div class="panel-warning"   id="delete_mylists" style="background-color: pink" >
                      My Device List Header box
                    ...
                    <table id="<?php echo $device['id'] ?>" style="background-color: lightgray" class="myswlist table table-border">
                      table list
                      <thead>
                        <tr>
                          <td width="64%"><b>List Name</b></td>
                          <td width ="2%"><b>Edit</b></td>
                          <td><b>View</b></td> 
                        </tr>
                      </thead>
                      <tbody> 
                  <?php  
                      ...
                      ?> 
                        <tr class="del_<?php echo $value['listid'];?>">                            
                         ...
                         <td width="15%">&nbsp;<a href="switchtech_devicelist.php?action=editmylist&switchlistid=<?php echo $value['listid'];?>"><i class="fa fa-edit"></i></a></td>
                          <!--
                          ...
                          <td>&nbsp;<a href="mylist_devices.php?userid=<?php echo $userid;?>&listid=<?php echo $value['listid'];?>"><i class="fa fa-eye" width="20" height="22"></i></a></td>
                        </tr>
                  <?php  
                      } 
                      ?> 
                ...
                <div class="col-md-6"  style="background-color: lightblue">
                  ...
                  <div style="border:1px solid darkgray;" >
                    ...
                    <div   class="panel-warning " style="background-color: pink" >
                      View Device List header
                      <div class="panel-heading" id="mylist_delete" style="background-color: pink">
                        <b>View Device List :</b> <?php echo $switchlist['mylistname'] ?>
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span type="button" style="background-color: none; " class="box box-danger"><i class="fa fa-trash"></i>Delete 
                        </span>
                      </div>
                    </div> 
                    <input type="hidden" name="hidd_mylistid" id="hidd_mylistid" value="<?php echo  $_SESSION['switchlistid']; ?>">
                    table list
                    <table id="deviceslist" class="droppable myswlist table table-border" <?php echo ($_SESSION['switchlistid']!='') ? 'data-mylistid="'.$_SESSION['switchlistid'] .'"':'' ?>  style="background-color: gray">
                      <thead><tr><td width="40%"><b> Device Id</b> </td><td><b>Device Name</b></td></tr></thead>
          ...
          <div class="col-md-6 panel-info pull-right" style="background-color: orange">
            Right side panel
            <div id="map_section" class="sec_with_map" style='<?php echo ($show_map_flag) ? "display: block" : "display: none" ?>; border:1px solid lightgray; background-color: gray' >
              map section 
             <div id="mapstates" style="background-color: skyblue"> 
                map area
                <img src="resources/img/map_new.png" usemap="#United States of America" border=0>
            ...
            <div class="panel-heading" style="background-color: blue">
               Routers list header section
               <form id="city_form" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="get" style="background-color: red"> 
                Search form
            <input type="hidden" name="marketname" id="marketname" value="<?php echo $city_name ?>">
              ...
              ?> 
                <input placeholder="Search coming soon" name="search_term" style="color: blue;" value="<?php echo $search_term; ?>" id="srch-term" type="text">
                ...
                <?php
                  ...
                  $switch_device_name = '';
                  $switch_list = get_switchlist_all_market($_SESSION['userid'], $_SESSION['marketname'], $_SESSION['city_name']); 
                  // print_r($switch_list );   
        ...
        <span id="map_hide_link" class="pull-right sec_with_map" style='cursor: pointer; <?php echo ($show_map_flag) ? "display: block" : "display: none" ?>' onclick="hideMap()"> Hide Map&nbsp;  <img width="25px" src="resources/img/usmap-icon.png" >&nbsp;</span>
            ...
            <div id="container_mymarketswitches" class="panel-body" style="background-color: lightblue">
              Router results section 
              
              <div style="border: 1px solid lightblue; padding: 15px;">
                <!-- Switch results without map -->
                <div id="sw_result_without_map" class="sec_without_map" style='<?php echo ($show_map_flag) ? "display: none" : "display: block" ?>; '>
                <?php
                  $high_limit = HIGH_LIMIT;
                  $low_limit = LOW_LIMIT;
                  if(isset($switch_device_name) && $switch_device_name != '' ) {
                    $page_limit = $high_limit;
                ...
                <!-- Switch results with map -->
                <div id="sw_result_with_map" class="sec_with_map" style='<?php echo ($show_map_flag) ? "display: block" : "display: none" ?>; '>
                  
                <?php
                $device_details= '';
                  if(isset($switch_device_name) && $switch_device_name != '' ) {
                    $page_limit = $low_limit;
                    if (!isset($str_marketname)) {
                      $device_details = getSWroutersDetails_all($switch_device_name, $search_term, $_SESSION['userid'],$page_limit);
                    }
                    else {
